...
function getWinnerIndex(handArr) {
  const scoredHands = handArr.map(x => scorePokerHand(x));
  return determineWinner(scoredHands);
}

function buildHands() {
  const hands = this.hand.order.map( id => {
    const playerHand = this.players[id].hand;
    const spread = this.hand.spread;
    return spread.concat(playerHand)
  })
  return hands;
}
function getWinningID() {
  const indices = this.getWinnerIndex(this.buildHands());
  return indices.map( index => {
    return this.hand.order[index]
  })
}
module.exports = {
  scorePokerHand,
